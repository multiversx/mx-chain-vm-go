...
func TestNewRuntimeContext(t *testing.T) {
	...
	runtimeContext, err := NewRuntimeContext(host, vmType, builtInFunctions.NewBuiltInFunctionContainer())
	require.Nil(t, err)
...
func TestTraceGas(t *testing.T) {
	...
	expectedGasTraceMap := map[string]map[string][]uint64{
		"firstScAddress": {
			"calledFunctionName": []uint64{0xbb80, 0xc350},
			"secondFunctionName": []uint64{0xbb8},
		},
	}
