...
`)
	...
	for _, funcMetadata := range eiMetadata.AllFunctions {
		...
		_, _ = out.WriteString(" {\n")
		_, _ = out.WriteString(fmt.Sprintf("    env.vm_hooks.%s(", snakeCase(funcMetadata.Name)))

		for i, arg := range funcMetadata.Arguments {
			if i > 0 {
				_, _ = out.WriteString(", ")
			}
			_, _ = out.WriteString(snakeCase(arg.Name))
		}

		_, _ = out.WriteString(")\n}\n\n")
	}

	_, _ = out.WriteString(`pub fn convert_imports_2(store: &Store, env: VMHooksWrapper) -> ImportObject {
    imports! {
        "env" => {
`)
