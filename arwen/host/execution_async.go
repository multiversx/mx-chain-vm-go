...
// executeAsyncContext is the entry-point of the async calling mechanism; it is
// called by host.ExecuteOnDestContext() and host.callSCMethod(). When
// executeAsyncContext() finishes, there should be no remaining AsyncCalls that
...
//
// executeAsyncContext() does NOT handle the callbacks of cross-shard
// AsyncCalls. See postprocessCrossShardCallback() for that
//
// Note that executeAsyncContext() is mutually recursive with
// host.ExecuteOnDestContext(), because synchronous AsyncCalls are executed
// with host.ExecuteOnDestContext(), which, in turn, calls
// host.executeAsyncContext() to resolve AsyncCalls generated by the
// AsyncCalls, and so on.
func (host *vmHost) executeAsyncContext() error {
...
func (host *vmHost) executeAsyncCallGroup(
	...
	if asyncCallGroup.IsCompleted() {
		// TODO reenable this, after allowing a gas limit for it (this method is
		// currently a NOP and simply returns nil)
...
func (host *vmHost) executeAsyncCall(
	...
	if execMode == arwen.SyncExecution {
		...
		_, err = host.executeSyncCallback(asyncCall, vmOutput, err)
		return err
	}
	...
	if execMode == arwen.AsyncBuiltinFunc {
		// Built-in functions handle cross-shard calls themselves, by generating
		// entries in vmOutput.OutputAccounts. It is not necessary to call
...
func (host *vmHost) determineExecutionMode(asyncCall *arwen.AsyncCall) (arwen.AsyncCallExecutionMode, error) {
	runtime := host.Runtime()
...
func (host *vmHost) createSyncCallInput(asyncCall arwen.AsyncCallInfoHandler) (*vmcommon.ContractCallInput, error) {
	runtime := host.Runtime()
...
func (host *vmHost) sendAsyncCallCrossShard(asyncCall arwen.AsyncCallInfoHandler) error {
	runtime := host.Runtime()
	output := host.Output()
