...
func (imports *EIFunctions) Append(importName string, implementation interface{}, cgoPointer unsafe.Pointer) (*EIFunctions, error) {
	...
	if _, duplicate := imports.FunctionMap[importName]; duplicate {
		return nil, NewImportedFunctionError(importName, "Duplicate imported function `%s`.")
	}
