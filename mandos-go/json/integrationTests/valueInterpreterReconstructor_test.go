...
import (
	...
	mvr "github.com/ElrondNetwork/arwen-wasm-vm/mandos-go/json/valuereconstructor"
...
func TestString(t *testing.T) {
	vi := mvi.ValueInterpreter{}
	vr := mvr.ValueReconstructor{}

	result, err := vi.InterpretString("``abcdefg")
	require.Nil(t, err)
	require.Equal(t, []byte("abcdefg"), result)
	require.Equal(t, "str:abcdefg", vr.Reconstruct(result, mvr.StrHint))

	result, err = vi.InterpretString("``")
	require.Nil(t, err)
	require.Equal(t, []byte{}, result)
	require.Equal(t, "str:", vr.Reconstruct(result, mvr.StrHint))
...
func TestAddress(t *testing.T) {
	...
	require.Equal(t, "address:", vr.Reconstruct(result, mvr.AddressHint))

	result, err = vi.InterpretString("address:a")
	require.Nil(t, err)
	require.Equal(t, []byte("a_______________________________"), result)
	require.Equal(t, "address:a", vr.Reconstruct(result, mvr.AddressHint))

	result, err = vi.InterpretString("address:a\x05")
	require.Nil(t, err)
	require.Equal(t, []byte("a\x05______________________________"), result)
	require.Equal(t, "address:a\x05", vr.Reconstruct(result, mvr.AddressHint))

	result, err = vi.InterpretString("address:an_address")
	require.Nil(t, err)
	require.Equal(t, []byte("an_address______________________"), result)
	require.Equal(t, "address:an_address", vr.Reconstruct(result, mvr.AddressHint))

	result, err = vi.InterpretString("address:1234567890123456789012345678901\x01")
	require.Nil(t, err)
	require.Equal(t, []byte("1234567890123456789012345678901\x01"), result)
	require.Equal(t, "address:1234567890123456789012345678901#01", vr.Reconstruct(result, mvr.AddressHint))

	// trims excess
	result, err = vi.InterpretString("address:1234567890123456789012345678901\x013")
	require.Nil(t, err)
	require.Equal(t, []byte("1234567890123456789012345678901\x01"), result)
	require.Equal(t, "address:1234567890123456789012345678901#01", vr.Reconstruct(result, mvr.AddressHint))
}
...
func TestAddressWithShardId(t *testing.T) {
	...
	result, err := vi.InterpretString("address:#05")
	require.Nil(t, err)
	require.Equal(t, []byte("_______________________________\x05"), result)
	require.Equal(t, "address:#05", vr.Reconstruct(result, mvr.AddressHint))

	result, err = vi.InterpretString("address:a#bb")
	require.Nil(t, err)
	require.Equal(t, []byte("a______________________________\xbb"), result)
	require.Equal(t, "address:a#bb", vr.Reconstruct(result, mvr.AddressHint))

	result, err = vi.InterpretString("address:an_address#99")
	require.Nil(t, err)
	require.Equal(t, []byte("an_address_____________________\x99"), result)
	require.Equal(t, "address:an_address#99", vr.Reconstruct(result, mvr.AddressHint))

	result, err = vi.InterpretString("address:1234567890123456789012345678901#66")
	require.Nil(t, err)
	require.Equal(t, []byte("1234567890123456789012345678901\x66"), result)
	require.Equal(t, "address:1234567890123456789012345678901#66", vr.Reconstruct(result, mvr.AddressHint))

	// trims excess
	result, err = vi.InterpretString("address:12345678901234567890123456789012#66")
	require.Nil(t, err)
	require.Equal(t, []byte("1234567890123456789012345678901\x66"), result)
	require.Equal(t, "address:1234567890123456789012345678901#66", vr.Reconstruct(result, mvr.AddressHint))
}
...
func TestSCAddress(t *testing.T) {
	...
	require.Equal(t, "sc:a", vr.Reconstruct(result, mvr.AddressHint))

	result, err = vi.InterpretString("sc:12345678901234567890120s")
	require.Nil(t, err)
	require.Equal(t, []byte("\x00\x00\x00\x00\x00\x00\x00\x0012345678901234567890120s"), result)
	require.Equal(t, "sc:12345678901234567890120#73", vr.Reconstruct(result, mvr.AddressHint))

	// trims excess
	result, err = vi.InterpretString("sc:12345678901234567890120sx")
	require.Nil(t, err)
	require.Equal(t, []byte("\x00\x00\x00\x00\x00\x00\x00\x0012345678901234567890120s"), result)
	require.Equal(t, "sc:12345678901234567890120#73", vr.Reconstruct(result, mvr.AddressHint))
}
...
func TestSCAddressWithShardId(t *testing.T) {
	...
	result, err := vi.InterpretString("sc:a#44")
	require.Nil(t, err)
	require.Equal(t, []byte("\x00\x00\x00\x00\x00\x00\x00\x00a______________________\x44"), result)
	require.Equal(t, "sc:a#44", vr.Reconstruct(result, mvr.AddressHint))

	result, err = vi.InterpretString("sc:12345678901234567890120#88")
	require.Nil(t, err)
	require.Equal(t, []byte("\x00\x00\x00\x00\x00\x00\x00\x0012345678901234567890120\x88"), result)
	require.Equal(t, "sc:12345678901234567890120#88", vr.Reconstruct(result, mvr.AddressHint))

	// trims excess
	result, err = vi.InterpretString("sc:12345678901234567890120x#88")
	require.Nil(t, err)
	require.Equal(t, []byte("\x00\x00\x00\x00\x00\x00\x00\x0012345678901234567890120\x88"), result)
	require.Equal(t, "sc:12345678901234567890120#88", vr.Reconstruct(result, mvr.AddressHint))
}
...
func TestUnsignedNumber(t *testing.T) {
	...
	require.Nil(t, err)
	require.Equal(t, []byte{}, result)
	require.Equal(t, "0", vr.Reconstruct(result, mvr.NumberHint))
	...
	require.Equal(t, "12", vr.Reconstruct(result, mvr.NumberHint))

	result, err = vi.InterpretString("256")
	require.Nil(t, err)
	require.Equal(t, []byte{0x01, 0x00}, result)
	require.Equal(t, "256", vr.Reconstruct(result, mvr.NumberHint))

	result, err = vi.InterpretString("0b1")
	require.Nil(t, err)
	require.Equal(t, []byte{0x01}, result)
	require.Equal(t, "1", vr.Reconstruct(result, mvr.NumberHint))
...
func TestSignedNumber(t *testing.T) {
	...
	require.Nil(t, err)
	require.Equal(t, []byte{0xff}, result)
	require.Equal(t, "255", vr.Reconstruct(result, mvr.NumberHint))
...
func TestSignedFixedWidth(t *testing.T) {
	...
	_, err = vi.InterpretString("i8:+255")
	...
	_, err = vi.InterpretString("i8:+0xff")
	require.NotNil(t, err)
...
func TestBigUint(t *testing.T) {
	...
	// should be positive
	_, err = vi.InterpretString("biguint:-0x01")
	require.NotNil(t, err)
...
func TestKeccak256(t *testing.T) {
	...
	require.Nil(t, err)
	expected, _ := mvi.Keccak256([]byte{0x01, 0x05})
	...
	require.Nil(t, err)
	expected, _ = mvi.Keccak256([]byte("ab"))
	require.Equal(t, expected, result)
