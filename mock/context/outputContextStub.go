...
type OutputContextStub struct {
	...
	TransferESDTCalled                func(destination []byte, originalCaller []byte, sender []byte, transfers []*vmcommon.ESDTTransfer, input *vmcommon.ContractCallInput) (uint64, error)
...
func (o *OutputContextStub) TransferESDT(destination []byte, originalCaller []byte, sender []byte, transfers []*vmcommon.ESDTTransfer, callInput *vmcommon.ContractCallInput) (uint64, error) {
	if o.TransferESDTCalled != nil {
		return o.TransferESDTCalled(destination, originalCaller, sender, transfers, callInput)
	}
	return 0, nil
}
