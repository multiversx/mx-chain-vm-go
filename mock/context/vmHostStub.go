...
type VMHostStub struct {
	...
	ExecuteESDTTransferCalled   func(destination []byte, originalCaller []byte, sender []byte, transfers []*vmcommon.ESDTTransfer, callType vm.CallType) (*vmcommon.VMOutput, uint64, error)
...
func (vhs *VMHostStub) ExecuteESDTTransfer(destination []byte, originalCaller []byte, sender []byte, transfers []*vmcommon.ESDTTransfer, callType vm.CallType) (*vmcommon.VMOutput, uint64, error) {
	if vhs.ExecuteESDTTransferCalled != nil {
		return vhs.ExecuteESDTTransferCalled(destination, originalCaller, sender, transfers, callType)
	}
	return nil, 0, nil
}
