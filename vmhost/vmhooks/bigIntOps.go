...
func (context *VMHooksImpl) BigIntGetUnsignedArgument(id int32, destinationHandle int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntGetUnsignedArgument
...
func (context *VMHooksImpl) BigIntGetSignedArgument(id int32, destinationHandle int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntGetSignedArgument
	...
	if enableEpochsHandler.IsFlagEnabled(vmhost.BarnardOpcodesFlag) {
		gasToUse = math.MulUint64(metering.GasSchedule().GetBaseOperationCost().DataCopyPerByte, uint64(len(args[id])))
...
func (context *VMHooksImpl) BigIntStorageStoreUnsigned(keyOffset executor.MemPtr, keyLength executor.MemLength, sourceHandle int32) int32 {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntStorageStoreUnsigned
...
func (context *VMHooksImpl) BigIntStorageLoadUnsigned(keyOffset executor.MemPtr, keyLength executor.MemLength, destinationHandle int32) int32 {
	...
	err = storage.UseGasForStorageLoad(bigIntStorageLoadUnsignedName,
		int64(trieDepth),
		metering.GasSchedule().GetBigIntAPICost().BigIntStorageLoadUnsigned,
		usedCache)
	if err != nil {
		context.FailExecution(err)
		return -1
...
func (context *VMHooksImpl) BigIntGetCallValue(destinationHandle int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntGetCallValue
	...
	value := managedType.GetBigIntOrCreate(destinationHandle)
	value.Set(runtime.GetVMInput().GetVMInput().CallValue)
}
...
func (context *VMHooksImpl) BigIntGetExternalBalance(addressOffset executor.MemPtr, result int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntGetExternalBalance
...
func (context *VMHooksImpl) BigIntNew(smallValue int64) int32 {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntNew
...
func (context *VMHooksImpl) BigIntUnsignedByteLength(referenceHandle int32) int32 {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntUnsignedByteLength
...
func (context *VMHooksImpl) BigIntSignedByteLength(referenceHandle int32) int32 {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntSignedByteLength
...
func (context *VMHooksImpl) BigIntGetUnsignedBytes(referenceHandle int32, byteOffset executor.MemPtr) int32 {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntGetUnsignedBytes
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return -1
	...
	gasToUse = math.MulUint64(metering.GasSchedule().GetBaseOperationCost().DataCopyPerByte, uint64(len(bytes)))
	err = metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return -1
...
func (context *VMHooksImpl) BigIntGetSignedBytes(referenceHandle int32, byteOffset executor.MemPtr) int32 {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntGetSignedBytes
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return -1
...
func (context *VMHooksImpl) BigIntSetUnsignedBytes(destinationHandle int32, byteOffset executor.MemPtr, byteLength executor.MemLength) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntSetUnsignedBytes
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func (context *VMHooksImpl) BigIntSetSignedBytes(destinationHandle int32, byteOffset executor.MemPtr, byteLength executor.MemLength) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntSetSignedBytes
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func (context *VMHooksImpl) BigIntIsInt64(destinationHandle int32) int32 {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntIsInt64
...
func (context *VMHooksImpl) BigIntSetInt64(destinationHandle int32, value int64) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntSetInt64
...
func (context *VMHooksImpl) BigIntAdd(destinationHandle, op1Handle, op2Handle int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntAdd
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func (context *VMHooksImpl) BigIntSub(destinationHandle, op1Handle, op2Handle int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntSub
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func (context *VMHooksImpl) BigIntMul(destinationHandle, op1Handle, op2Handle int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntMul
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func (context *VMHooksImpl) BigIntTDiv(destinationHandle, op1Handle, op2Handle int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntTDiv
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func (context *VMHooksImpl) BigIntTMod(destinationHandle, op1Handle, op2Handle int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntTMod
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func (context *VMHooksImpl) BigIntEDiv(destinationHandle, op1Handle, op2Handle int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntEDiv
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func (context *VMHooksImpl) BigIntEMod(destinationHandle, op1Handle, op2Handle int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntEMod
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func (context *VMHooksImpl) BigIntSqrt(destinationHandle, opHandle int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntSqrt
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func (context *VMHooksImpl) BigIntPow(destinationHandle, op1Handle, op2Handle int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntPow
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func (context *VMHooksImpl) BigIntLog2(op1Handle int32) int32 {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntLog
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return -1
...
func (context *VMHooksImpl) BigIntAbs(destinationHandle, opHandle int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntAbs
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func (context *VMHooksImpl) BigIntNeg(destinationHandle, opHandle int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntNeg
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func (context *VMHooksImpl) BigIntSign(opHandle int32) int32 {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntSign
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return -2
...
func (context *VMHooksImpl) BigIntCmp(op1Handle, op2Handle int32) int32 {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntCmp
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return -2
...
func (context *VMHooksImpl) BigIntNot(destinationHandle, opHandle int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntNot
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func (context *VMHooksImpl) BigIntAnd(destinationHandle, op1Handle, op2Handle int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntAnd
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func (context *VMHooksImpl) BigIntOr(destinationHandle, op1Handle, op2Handle int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntOr
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func (context *VMHooksImpl) BigIntXor(destinationHandle, op1Handle, op2Handle int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntXor
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func (context *VMHooksImpl) BigIntShr(destinationHandle, opHandle, bits int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntShr
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func (context *VMHooksImpl) BigIntShl(destinationHandle, opHandle, bits int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntShl
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func (context *VMHooksImpl) BigIntFinishUnsigned(referenceHandle int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntFinishUnsigned
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
	...
	gasToUse = math.MulUint64(metering.GasSchedule().GetBaseOperationCost().PersistPerByte, uint64(len(value.Bytes())))
	err = metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func (context *VMHooksImpl) BigIntFinishSigned(referenceHandle int32) {
	...
	gasToUse := metering.GasSchedule().GetBigIntAPICost().BigIntFinishSigned
	err := metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
	...
	gasToUse = math.MulUint64(metering.GasSchedule().GetBaseOperationCost().PersistPerByte, uint64(len(bigInt2cBytes)))
	err = metering.UseGasBounded(gasToUse)
	if err != nil {
		context.FailExecution(err)
		return
...
func BigIntToStringWithHost(host vmhost.VMHost, bigIntHandle int32, destinationHandle int32) {
	...
	gasToUse = math.MulUint64(metering.GasSchedule().GetBaseOperationCost().DataCopyPerByte, uint64(len(resultStr)))
	err = metering.UseGasBounded(gasToUse)
	if err != nil {
		FailExecution(host, err)
		return
