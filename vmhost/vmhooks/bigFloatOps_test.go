...
func TestVMHooksImpl_BigFloatCmp(t *testing.T) {
	t.Parallel()
	hooks, _, _, metering, _, _ := createTestVMHooks()
	metering.On("UseGasBoundedAndAddTracedGas", mock.Anything, mock.Anything).Return(nil)
...
func TestVMHooksImpl_BigFloatPow(t *testing.T) {
	...
	managedType := &context.MockManagedTypesContext{}
	host.(*context.MockVMHost).On("ManagedTypes").Return(managedType)
	host.(*context.MockVMHost).On("EnableEpochsHandler").Return(&worldmock.EnableEpochsHandlerStub{})
