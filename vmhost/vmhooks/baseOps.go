...
// GetBlockRoundTimeInMilliseconds VMHooks implementation.
// @autogenerate(VMHooks)
func (context *VMHooksImpl) GetBlockRoundTimeInMilliseconds() int64 {
	blockchain := context.GetBlockchainContext()
	metering := context.GetMeteringContext()

	gasToUse := metering.GasSchedule().BaseOpsAPICost.GetRoundTime
	err := metering.UseGasBoundedAndAddTracedGas(getBlockRoundTimeInMillisecondsName, gasToUse)
	if err != nil {
		context.FailExecution(err)
		return -1
	}
