...
func (context *storageContext) setStorageToAddress(address []byte, key []byte, value []byte) (vmhost.StorageStatus, error) {
	if err := context.checkReservedAndProtection(key); err != nil {
	...
	if err = context.consumeGasForKey(key, usedCache); err != nil {
		return vmhost.StorageUnchanged, err
	}

	if bytes.Equal(oldValue, value) {
		return context.handleUnchangedStorage(len(value), usedCache, key)
	}
...
func (context *storageContext) handleUnchangedStorage(length int, usedCache bool, key []byte) (vmhost.StorageStatus, error) {
