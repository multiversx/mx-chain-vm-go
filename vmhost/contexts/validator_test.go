...
func TestFunctionsGuard_isValidFunctionName(t *testing.T) {
	...
	validator := newWASMValidator(testImportNames(), builtInFuncContainer, &worldmock.EnableEpochsHandlerStub{})
...
func TestFunctionsProtected(t *testing.T) {
	...
	validator := newWASMValidator(testImportNames(), builtInFunctions.NewBuiltInFunctionContainer(), &worldmock.EnableEpochsHandlerStub{})
