...
func TestFunctionsGuard_isValidFunctionName(t *testing.T) {
	...
	validator := newWASMValidator(true, testImportNames(), builtInFuncContainer)
...
func TestFunctionsProtected(t *testing.T) {
	...
	validator := newWASMValidator(true, testImportNames(), builtInFunctions.NewBuiltInFunctionContainer())
