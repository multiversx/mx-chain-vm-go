...
func TestFunctionsGuard_isValidFunctionName(t *testing.T) {
	...
	validator := newWASMValidator(true, testImportNames(), builtInFuncContainer, worldmock.EnableEpochsHandlerStubAllFlags())
...
func TestFunctionsProtected(t *testing.T) {
	...
	validator := newWASMValidator(true, testImportNames(), builtInFunctions.NewBuiltInFunctionContainer(), worldmock.EnableEpochsHandlerStubAllFlags())
